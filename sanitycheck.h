...
static const __m256i bit_mask = _mm256_set_epi64x(1<<24,1<<16,1<<8,1<<0);

inline auto get_mask(const uint32_t mask)
{
 __m256i vmask = _mm256_and_si256(_mm256_set1_epi64x(mask), bit_mask); 
 return _mm256_cmpeq_epi64(vmask, bit_mask);;
}
 ...
 inline uint64_t dot_prod(const std::vector<__m256i> rnd0_[4], size_t shift0, const std::vector<__m256i> rnd1_[4], size_t shift1, const uint8_t * buf)
{
  ...
  for (size_t i = 0, k = 0; i < rnd1.size()/4; ++i)
  {
    ...
    for (size_t j = 0; j < rnd0.size(); j+=4, k+=4)
    {
        inner[0] = _mm256_add_epi64(inner[0], _mm256_and_si256(rnd0[j+0], get_mask(buf32[k+0])));
        inner[1] = _mm256_add_epi64(inner[1], _mm256_and_si256(rnd0[j+1], get_mask(buf32[k+1])));
        inner[2] = _mm256_add_epi64(inner[2], _mm256_and_si256(rnd0[j+2], get_mask(buf32[k+2])));
        inner[3] = _mm256_add_epi64(inner[3], _mm256_and_si256(rnd0[j+3], get_mask(buf32[k+3])));
    }
