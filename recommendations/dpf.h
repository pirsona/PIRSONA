...
template<typename leaf_t, typename node_t, typename prgkey_t>
inline void evalinterval(const dpf_key<leaf_t, node_t, prgkey_t> & dpfkey, const size_t from, const size_t to, leaf_t * output, uint8_t * t = NULL, bool zero = false);
...
struct dpf_key final
{
  public:
	...
	inline void evalinterval(const size_t from, const size_t to, leaf_t * output, uint8_t * t = NULL, bool zero = false) const { dpf::evalinterval(*this, from, to, output, t, zero); }
...
template<typename leaf_t, typename node_t, typename prgkey_t>
inline void __evalinterval(const dpf_key<leaf_t, node_t, prgkey_t> & dpfkey, const size_t from, const size_t to, leaf_t * output, uint8_t * _t, bool zero)
{
	...
	if (zero)
	{
auto lsb = bit_mask_avx2_msb(128-(from % 128));
auto msb = bit_mask_avx2_lsb(to % 128);
//printf("%lx,%lx\n", lsb[1],lsb[0]);
//printf("%lx,%lx\n", msb[1],msb[0]);
		s[0][0] = _mm_and_si128(s[0][0], lsb);
		s[0][nodes_in_interval-1] = _mm_and_si128(s[0][nodes_in_interval-1], msb);
	}
} // dpf::__evalinterval

template<typename leaf_t, typename node_t, typename prgkey_t>
inline void evalinterval(const dpf_key<leaf_t, node_t, prgkey_t> & dpfkey, const size_t from, const size_t to, leaf_t * output, uint8_t * t, bool zero)
{
	uint8_t * tt = t ? t : reinterpret_cast<uint8_t *>(malloc(dpfkey.nodes_in_interval(from, to) * sizeof(uint8_t)));
	__evalinterval(dpfkey, from, to, output, tt, zero);
	if (!t) free(tt);
} // dpf::evalinterval
